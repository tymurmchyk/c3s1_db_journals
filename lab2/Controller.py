...
class Controller:
	...
	def edit_account(self):
		...
		while True:
			...
			try:
				id = int(id)
				if self.model.get_account(id) is None:
	...
	def show_account(self):
		...
		temp = self.model.get_account(id)
		self.view.table([[temp.id, temp.email, temp.password, temp.name]], ["id", "email", "password", "name"])

	def show_all_accounts(self):
		self.view.message("all accounts")
		temp = self.model.get_all_accounts()
		accs = []
		for acc in temp:
			accs.append([acc.id, acc.email, acc.password, acc.name])
		self.view.table(accs, ["id", "email", "password", "name"])
	...
	def create_journal(self):
		...
		while True:
			...
			try:
				...
				if self.model.get_account(account_id) is None:
					self.view.message("account with such id doesn't exist")
					continue
			except:
	...
	def edit_journal(self):
		...
		while True:
			...
			try:
				id = int(id)
				if self.model.get_journal(id) is None:
	...
	def delete_journal(self):
		...
		while True:
			...
			self.view.message(
				...
				f"name: {jrnl.name}\n"
				f"account id: {jrnl.account_id}\n"
				f"created_on: {jrnl.created_on}\n"
				f"last_edit: {jrnl.last_edit}"
				)
			while True:
				option = self.view.get_input("are you sure (y/n)? ").lower()
	...
	def show_journal(self):
		...
		temp = self.model.get_journal(id)
		self.view.table([[temp.id, temp.name, temp.created_on, temp.last_edit, temp.account_id]], ["id", "name", "created on", "last edit", "account id"])

	def show_all_journals(self):
		self.view.message("all journals")
		temp = self.model.get_all_journals()
		jrnls = []
		for jrnl in temp:
			jrnls.append([jrnl.id, jrnl.name, jrnl.created_on, jrnl.last_edit, jrnl.account_id])
		self.view.table(jrnls, ["id", "name", "created on", "last edit", "account id"])
	...
	def create_note(self):
		...
		while True:
			...
			try:
				...
				if self.model.get_journal(journal_id) is None:
					self.view.message("journal with such id doesn't exist")
					continue
			except:
	...
	def edit_note(self):
		...
		while True:
			...
			try:
				id = int(id)
				if self.model.get_note(id) is None:
	...
	def delete_note(self):
		...
		while True:
			...
			self.view.message(
				...
				f"name: {note.name}\n"
				f"date: {note.date}\n"
				f"added on: {note.added_on}\n"
				f"journal id: {note.journal_id}\n"
				f"text:\n{note.text}"
				)
			while True:
				option = self.view.get_input("are you sure (y/n)? ").lower()
	...
	def show_note(self):
		...
		temp = self.model.get_note(id)
		self.view.table([temp.id, temp.name, temp.date, temp.added_on, temp.journal_id, temp.text], ["id", "name", "date", "added_on", "journal id", "text"])
	...
	def show_all_notes(self):
		...
		if len(notes) != 0:
			...
			nots = []
			for note in temp:
				nots.append([note.id, note.name, note.date, note.added_on, note.journal_id])
